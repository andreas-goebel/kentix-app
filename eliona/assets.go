...
// CreateDoorlockAssetsIfNecessary create all assets for specification if not already exists
...
// createDoorlockAssetIfNecessary create asset for specification if not already exists
func createDoorlockAssetIfNecessary(config apiserver.Configuration, projectId string, parentAssetId *int32, spec kentix.DoorLock) error {
	...
	// If no asset exists for project and configuration, create a new one
	name := doorlockName(spec)
	description := doorlockDescription(spec)
	...
	log.Debug("eliona", "Created new asset for project %s and doorlock %s.", projectId, spec.Serial)

	return nil
}

func doorlockName(specification kentix.DoorLock) string {
	return fmt.Sprintf("%s (%s)", specification.Name, specification.Address)
}

func doorlockDescription(specification kentix.DoorLock) string {
	return fmt.Sprintf("%s (%s)", specification.Name, specification.Serial)
}
